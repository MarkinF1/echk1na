...
class DataBase:
    ...
    def get_ready_data_special(self, id_train: int, unit: int, direction: int, max_date: datetime.datetime,
                               min_date: datetime.datetime, arr_idx: Optional[int] = None) -> List[EchkinaReadyTable]:
        """
        ...
        with self.session() as session:
            condition = and_(
                EchkinaReadyTable.id_train == id_train,
                EchkinaReadyTable.unit == unit,
                EchkinaReadyTable.direction == direction,
                EchkinaReadyTable.date <= max_date,
                EchkinaReadyTable.date >= min_date
            )
            if arr_idx is not None:
                condition = and_(condition, EchkinaReadyTable.arr_idx == arr_idx)
